...
#
# 저자 확인 결과 기준 산출물이 **10.666** 이므로 「방법」 칸이 옳고
# 예시 쪽이 틀렸다. 구현은 처음부터 「방법」 칸을 따랐으므로 변경하지 않는다.
# 명세의 예시 수치는 10.666 기준으로 정정되었다.
#
# ⚠ 2026-08-19 출처 정정 — 여기 적혀 있던 근거 파일명 `result.xlsx` 는 틀렸다.
# 그 파일의 「경제적 관경계산」 시트는 헤더만 있고 값이 없다(실측 확인). 10.666 을
# 뒷받침하는 23행이 들어 있는 것은 **「BIM 기반 사업부지 검토 요약 보고서.xlsx」**
# 쪽이다. `experiments/exp_baseline_recompute.py` 가 그 파일을 읽어 원 워크플로의
# 산식을 재현하며, 유속·손실수두·총현가 23행이 전부 일치함을 확인한다.
#
