...
#
# ⚠ 2026-08-16 확인 — 명세 안에 불일치가 있다.
#   · 「방법」 칸의 공식은 계수 10.666 · 지수 1.85
#   · 그런데 검산 예시(Δh 15.43, 동수경사 0.003674)는 **10.67** 로 계산된 값이다
#     (10.666 -> 15.4234 / 0.003672, 10.67 -> 15.4292 / 0.003674).
# 차이는 0.04% 로 작으나 **계통 오차**라 5장 기준값 대조에서 그대로 남는다.
# 여기서는 정본의 「방법」 칸을 따라 10.666 을 쓰되, 실제로 적용한 상수를
# 출력에 실어 어느 관례로 나온 값인지 대조할 수 있게 한다.
#
# 참고: 국제적으로 널리 쓰이는 SI 형태는 계수 10.67 · 지수 1.852 이고,
# 10.666 · 1.85 는 국내 실무에서 흔한 반올림 변형이다. result.xlsx 의
# 「관로 수리계산」 시트가 어느 쪽인지 확인한 뒤 필요하면 여기를 맞춘다.
